...
export function Modal({ open, onClose, children, side = "center", size = "md", title, description, className, }) {
    useEffect(() => {
        if (!open)
            return;
        const onKey = (e) => {
    ...
    return (<div className="fixed inset-0 z-50" role="dialog" aria-modal="true" aria-label={title}>
      <button aria-label="Close overlay" onClick={onClose} className="absolute inset-0 bg-foreground/40 backdrop-blur-sm animate-in fade-in-0"/>
      <div data-state="open" className={cn("absolute flex flex-col overflow-hidden border border-border bg-card shadow-xl", panel, className)}>
        {(title || description) && (<div className="flex items-start justify-between gap-4 border-b border-border p-4 sm:p-5">
