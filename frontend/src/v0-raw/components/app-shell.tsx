...
export type ViewKey = "dashboard" | "patients" | "triage" | "referrals" | "model"
      ...
      {NAV.map(({ key, label, icon: Icon }) => {
        ...
        return (
          ...
          >
            ...
            {key === "referrals" && pendingReferrals > 0 && (
              <span className="flex size-5 items-center justify-center rounded-full bg-critical text-[10px] font-bold text-critical-foreground">
                {pendingReferrals}
              </span>
            )}
          </button>
...
function TopBar({ view, onMenu }: { view: ViewKey; onMenu: () => void }) {
      ...
      <button
        ...
        className={cn(
          ...
          online
            ...
            : "border-moderate/30 bg-moderate-muted text-moderate",
        )}
