...
import type { Patient, RiskZone } from "@/lib/types"
...
const FILTERS: { key: RiskZone | "all"; label: string }[] = [
  { key: "all", label: "All" },
...
export function PatientsView({ onTriage }: { onTriage: (patientId?: string) => void }) {
  const { patients } = useStore()
  const [query, setQuery] = useState("")
  const [filter, setFilter] = useState<RiskZone | "all">("all")
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-1.5">
                  ...
                  {!p.synced && (
                    <CloudOff className="size-3.5 shrink-0 text-moderate" aria-label="Not synced" />
                  )}
                </div>
      ...
      <form onSubmit={submit} className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <NumberField
            ...
            min={4}
            max={44}
            onChange={(v) => update("gestationalWeeks", v)}
          />
          <NumberField
            label="Gravidity"
        ...
        <label className="flex items-center justify-between rounded-xl border border-border p-3">
          ...
          <Switch
            checked={form.previousPreeclampsia}
            onCheckedChange={(v) => update("previousPreeclampsia", v)}
          />
        </label>

        {syncState === "offline" && (
          <p className="flex items-center gap-2 rounded-lg bg-moderate-muted px-3 py-2 text-xs text-moderate">
...
function PatientDetail({
  ...
  return (
    <Modal open={!!patient} onClose={onClose} title={patient.fullName} size="lg">
      <div className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-2">
          ...
          {records.length > 0 ? (
            ...
            <p className="text-sm text-muted-foreground">No recorded visits.</p>
          )}
        </div>
