        ...
        <Card>
          ...
          <CardContent>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    ...
                    strokeWidth={0}
                  >
                    {dashboard.riskDistribution.map((d) => (
                      <Cell key={d.zone} fill={RISK_COLORS[d.zone]} />
                    ))}
                  </Pie>
            ...
            <div className="mt-4 flex flex-col gap-2">
              {dashboard.riskDistribution.map((d) => (
                <div key={d.zone} className="flex items-center gap-2 text-sm">
                  <span
                    className="size-2.5 rounded-full"
                    style={{ background: RISK_COLORS[d.zone] }}
                  />
          ...
          <CardContent className="flex flex-col gap-2">
            {patients
              ...
              .map((p) => (
                ...
                >
                  <span
                    className={`flex size-9 shrink-0 items-center justify-center rounded-full text-sm font-semibold ${riskClasses(p.riskZone).badge}`}
                  >
                    {initials(p.fullName)}
                  </span>
        ...
        <Card>
          ...
          <CardContent>
            <div className="h-56 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
                  data={dashboard.topSyndromes.map((s) => ({
