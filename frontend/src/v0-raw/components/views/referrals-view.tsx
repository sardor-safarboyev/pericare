...
import type { Referral, ReferralActionType, ReferralStatus } from "@/lib/types"
...
const ACTION_ICONS: Record<ReferralActionType, typeof Pill> = {
...
export function ReferralsView() {
  ...
  const [actionFor, setActionFor] = useState<Referral | null>(null)
  ...
  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-4">
      ...
      {sorted.map((r) => (
        <ReferralCard key={r.id} referral={r} onAddAction={() => setActionFor(r)} />
      ))}
      ...
      <CardContent className="flex flex-col gap-4 p-4">
        ...
        <div className="flex items-center gap-2">
          {STATUS_FLOW.map((status, i) => {
            ...
            return (
              <div key={status} className="flex flex-1 items-center gap-2 last:flex-none">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {nextStatus && (
            <Button
              size="sm"
              onClick={() => advanceReferral(referral.id, nextStatus)}
              className="gap-1.5"
            >
...
function ActionModal({
  ...
  const [type, setType] = useState<ReferralActionType>("magnesium_sulfate")
  ...
  return (
    <Modal open={!!referral} onClose={onClose} title={`Log action · ${referral.patientName}`}>
      <form onSubmit={submit} className="flex flex-col gap-4">
        <div className="flex flex-col gap-1.5">
          ...
          <Select
            id="action-type"
            value={type}
            onChange={(e) => setType(e.target.value as ReferralActionType)}
          >
            {(Object.keys(REFERRAL_ACTION_LABELS) as ReferralActionType[]).map((t) => (
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <Textarea
            ...
            placeholder="Dose, time, responsible clinician…"
            rows={3}
          />
        </div>
