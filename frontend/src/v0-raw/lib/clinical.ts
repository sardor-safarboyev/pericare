...
export const SYNDROME_LABELS: Record<ObstetricSyndrome, string> = {
...
export const PROTEINURIA_LEVELS: ProteinuriaLevel[] = ["negative", "trace", "1+", "2+", "3+", "4+"]
...
const PROTEINURIA_SCORE: Record<ProteinuriaLevel, number> = {
  negative: 0,
  trace: 1,
  "1+": 2,
  "2+": 3,
...
export function shockIndexBand(si: number): { label: string; zone: RiskZone } {
  if (si < 0.7) return { label: "Normal", zone: "green" }
  if (si <= 0.9) return { label: "Borderline", zone: "yellow" }
 ...
 */
export function evaluateVitals(vitals: Vitals, previousPreeclampsia = false): Omit<
  TriageResult,
  "id" | "evaluatedAt" | "patientId"
> {
  ...
  if (probability >= 0.7) riskZone = "red"
  else if (probability >= 0.45) riskZone = "yellow"

  let syndrome: ObstetricSyndrome = "none"
  if (vitals.spo2 < 90 && vitals.heartRate > 110) syndrome = "postpartum_hemorrhage"
  else if (vitals.temperature >= 38.5 && vitals.heartRate > 100) syndrome = "sepsis"
  else if (vitals.systolic >= 160 && protein >= 3) syndrome = "eclampsia"
  else if (vitals.systolic >= 160 || vitals.diastolic >= 110) syndrome = "severe_preeclampsia"
  else if ((vitals.systolic >= 140 || vitals.diastolic >= 90) && protein >= 1) syndrome = "preeclampsia"
  else if (vitals.systolic >= 140 || vitals.diastolic >= 90) syndrome = "gestational_hypertension"
